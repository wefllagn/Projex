...
The separately approved Phase 11A dependency-remediation commit updated the client to React Router DOM/Router `7.18.2`, PostCSS `8.5.26`, brace-expansion `5.0.9`, and NanoID `3.3.18`. The full client audit and primary-advisory reconciliation were clear at that acceptance boundary. Future dependency checking remains part of pre-deployment acceptance rather than an authorization for continuous unrelated upgrades.

## Deferred boundaries

Phase 11B.1 owns guarded operational tooling, release/process templates, and the canonical runbook. Phase 11B.2 owns an explicitly approved isolated paired backup/restore proof. Phase 11C owns controlled hosted deployment and exposure. Phase 11D owns acceptance, load, security evidence, and capstone evaluation. Hosted Java and Git execution remain unavailable until their isolation boundaries are separately approved and verified.
