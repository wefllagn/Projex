...
Phase 10A completed shared infrastructure, authentication, classes, and memberships. Phase 10B completed activity/test-case authoring, student practice/submissions, and instructor assessment/release. Phase 10C.1 completed project-task and repository-foundation integration. Phase 10C.2 completed collaboration, membership, academic review, feedback, and repository archive/restore integration. Git inspection/credentials and admin data remain later boundaries. Backend records are authoritative; production routes never fall back to mocks after an API failure.
...
| `/student/projects/:projectTaskId/repositories/:repositoryId` | `RepositoryFoundationDetail` | Connected through 10C.2 | Real metadata, provisioning, safe members, owner invitations/member actions, review submission, released feedback, and lifecycle truth; Git controls deferred | Repository detail/update/collaboration/review | 10C.2 complete | Cross-resource identity and role-safe projections are verified | Fake collaboration/review actions retired; Git references remain isolated |
...
| `/student/repositories/:repositoryId` | `RepositoryFoundationDetail` | Connected through 10C.2 | Real detail, owner metadata, provisioning, membership, review/released feedback, and authorized personal lifecycle | Repository detail/update/collaboration/review/lifecycle | 10C.2 complete | No source, credential, path, draft-feedback, or account-detail exposure | Fixed identity and fake review state retired |
...
| `/instructor/projects/:projectTaskId/repositories/:repositoryId` | `RepositoryFoundationDetail` | Connected through 10C.2 | Real repository metadata, detailed membership, corrective actions, invitations, draft/released feedback, review transitions, approval, and archive/restore; Git controls deferred | Repository detail/collaboration/review/lifecycle | 10C.2 complete | Project/repository identity and independent repository/member/feedback versions are verified | Fake source remains isolated; fake invitation/review actions retired |
...
`client/src/data/projexData.js` retains roles, course/section options, the route catalog, non-authoritative Git visual references, analytics/archive records, instructor contribution/similarity/student-profile records, and all admin tables/summaries. Phase 10B retired activity, workspace, submission, score, feedback, and instructor assessment fixtures from authoritative routes. Phase 10C.1 retired fixed project-task/repository identity and foundation records; Phase 10C.2 retires local collaboration/review behavior while unreachable Git-inspection references remain for 10C.3.

### Student page inline records

Phase 10A.2 retires student class/home/stream/invitation/roster fixtures. Phase 10B retires inline activity, coding workspace, submission, score, feedback, and assessment records. Phase 10C.1 routes use real project/repository records and dynamic identifiers. Phase 10C.2 uses the real class roster, repository invitations, membership, feedback, and review contracts; old mixed repository workspace functions remain unreachable solely as Git-layout references for 10C.3 and are never attached to a real record after an API failure.

### Instructor page inline records

Phase 10A.2 retires `instructorClass`, class dashboard/home cards and statistics, stream items, browser join-code generation, and the old roster. Phase 10B removes the inline activity catalog, local create/settings behavior, and instructor assessment fixtures. Phase 10C.1 disconnects project-task/repository foundation fixtures from production routes; Phase 10C.2 replaces collaboration/review behavior with real contracts. Git visuals remain assigned to 10C.3 and contribution/similarity fixtures remain deferred.
...
- Phase 10C.2 authoritative repository routes omit browser file upload/edit, branch/tag creation, ZIP download, IDE launch, and Git mutation. Old Git visual references remain unreachable pending 10C.3 retirement.
...
Phase 10A, Phase 10B, Phase 10C.1, and Phase 10C.2 are complete on `phase/10-frontend-integration`; the remaining bullets describe milestone ownership rather than incomplete status.
...
Within Phase 10C, **10C.1 project-task and repository foundation is complete and committed**: selected-class catalogs/details, supported authoring and lifecycle with optimistic concurrency, student personal/class repository creation, role-scoped catalogs, repository metadata, bounded provisioning polling, archived presentation, and cross-resource identity checks. **10C.2 collaboration/review is complete**: student-safe membership and invitations, owner membership management, review submission/resubmission, instructor corrective actions, version-safe feedback drafts, request changes, approval, released feedback, and authorized repository archive/restore. **10C.3 Git inspection/credentials** remains a separate future implementation boundary.
