...
| Student programming workspace and visible-test practice | Yes | Yes | Core | INTEGRATED | Phase 11 stabilization | Real selected-activity starter source, memory-only editing, guarded practice requests, bounded polling, and accessible desktop pane resizing with stacked narrow-screen behavior |
...
| Repository Git summary, branches, commits, tree, blob, and diff inspection | Yes | Yes | Core | INTEGRATED | Phase 11 stabilization | Authorized lazy-loaded read-only Git projections with bounded binary, missing, and limit states; role-aware empty guidance and explicit refresh keep externally pushed content current |
| Repository-scoped Git credentials and local-client guidance | Yes | Yes | Core | INTEGRATED | Phase 11 stabilization | Short-lived issuance/list/revoke, one-time memory-only secret, credential-free local Git commands, owner/lead initial-main guidance, and an honest unavailable state when Smart HTTP is disabled |
...
| `/student/activity/:activityId/workspace` | `StudentProgrammingWorkspace` | Connected | Real activity starter source, memory-only editing, visible-test execution, authoritative submission availability, immutable confirmation, ambiguous-request recovery, and pointer/keyboard pane resizing | Visible-test run, attempt-state, and submission APIs | Phase 11 stabilization | Instructions, editor, and diagnostics keep sensible desktop bounds; separators disappear when the workspace stacks at 1180px and below | History scanning, fake files, autosave, compiler success, custom input, fixed IDs, and one-attempt claim retired |
...
| `/instructor/activity/:activityId/submissions/:submissionId` | `InstructorSubmissionReview` | Baseline connected; fresh rerun is unaccepted WIP on task branch | Immutable source/evidence, credited-result context, separate fresh review rerun, append-only correction, review draft, release, retry, and failure resolution | Get/review-run/correct/review/release/retry/resolve | Iteration 2 Instructor rerun WIP | Existing instructor visual language around explicit projection and lifecycle sections | Fake compiler, per-test edits, similarity, grade, release, and reopening retired; rerun never alters original evidence; normal migration and authenticated walkthrough pending |
